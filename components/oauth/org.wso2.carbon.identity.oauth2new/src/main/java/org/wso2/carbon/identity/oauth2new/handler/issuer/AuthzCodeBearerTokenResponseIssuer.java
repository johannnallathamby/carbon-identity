...
public class AuthzCodeBearerTokenResponseIssuer extends BearerTokenResponseIssuer {

    private static Log log = LogFactory.getLog(AuthzCodeBearerTokenResponseIssuer.class);

    @Override
    public OAuth2TokenResponse issueTokenResponse(OAuth2TokenMessageContext messageContext) throws OAuth2Exception {

        OAuth2TokenResponse response = super.issueTokenResponse(messageContext);
        return response;
    }

    public boolean canIssue(MessageContext messageContext) {
